...
func looksLikeDashboardHTML(body []byte) bool {
	...
	return strings.Contains(lower, "<!doctype html") ||
		strings.Contains(lower, "<title>freshworks</title>") ||
		strings.Contains(lower, "freshworks-products-sidebar")
}

// agentNameFromMe extracts a friendly identifier from /agents/me so doctor
// can confirm "authenticated as <person>" without dumping the whole record.
func agentNameFromMe(body []byte) string {
	var envelope struct {
		Agent struct {
	...
	switch {
	case envelope.Agent.Email != "":
		return envelope.Agent.Email
	case envelope.Agent.FirstName != "" || envelope.Agent.LastName != "":
		return strings.TrimSpace(envelope.Agent.FirstName + " " + envelope.Agent.LastName)
	}
	return ""
}
...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  freshservice-pp-cli doctor
  freshservice-pp-cli doctor --json
  freshservice-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export FRESHSERVICE_APIKEY=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("FRESHSERVICE_APIKEY") != "" {
				authEnvSet = append(authEnvSet, "FRESHSERVICE_APIKEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "FRESHSERVICE_APIKEY")
			}
			if os.Getenv("FRESHSERVICE_DOMAIN") != "" {
				authEnvSet = append(authEnvSet, "FRESHSERVICE_DOMAIN")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "FRESHSERVICE_DOMAIN")
			}
			switch {
				...
				if clientErr != nil {
					...
					if strings.Contains(cfg.BaseURL, ".myfreshworks.com") {
						report["domain_hint"] = "FAIL: " + cfg.BaseURL +
							...
							"Set FRESHSERVICE_DOMAIN to <tenant>.freshservice.com (often the org slug without the '-org' suffix)."
					}

					// Step 2: Validate credentials with an authenticated probe.
					// GET /agents/me is the canonical "who am I" endpoint —
					// cheap, read-only, and the response distinguishes
					// authenticated (200 with {"agent": {...}}) from broken
					// auth (401 / 403) and from wrong-host (200 with HTML).
					...
					if authHeader == "" {
						...
						body, probeErr := c.Get("/agents/me", nil)
						var probeAPIErr *client.APIError
						switch {
						case probeErr == nil:
							if looksLikeDashboardHTML(body) {
								report["credentials"] = "FAIL: /agents/me returned HTML, not JSON. " +
									"FRESHSERVICE_DOMAIN is probably pointed at the Freshworks org dashboard " +
									"(e.g. <org>.myfreshworks.com) instead of the Freshservice tenant " +
									"(<tenant>.freshservice.com)."
							} else if agent := agentNameFromMe(body); agent != "" {
								report["credentials"] = "OK (authenticated as " + agent + ")"
							} else {
								report["credentials"] = "OK (authenticated)"
							}
						case errors.As(probeErr, &probeAPIErr):
							switch probeAPIErr.StatusCode {
							case 401, 403:
								// Freshservice returns both 401 and 403 for bad
								// credentials depending on key state; treat them
								// as the same diagnostic so users see one clear
								// remediation path.
								report["credentials"] = fmt.Sprintf(
									"FAIL: HTTP %d on /agents/me — credentials rejected. "+
										"Check FRESHSERVICE_APIKEY (Profile Settings → API Settings in Freshservice). "+
										"If the key is correct, verify the key has agent-level access; "+
										"requester-only keys cannot read /agents/me.",
									probeAPIErr.StatusCode,
								)
							case 404:
								report["credentials"] = "FAIL: 404 at /agents/me — base URL is wrong. " +
									"Expected https://<tenant>.freshservice.com/api/v2; got " + cfg.BaseURL + "."
							default:
								report["credentials"] = fmt.Sprintf("FAIL: HTTP %d on /agents/me", probeAPIErr.StatusCode)
							}
						default:
							report["credentials"] = fmt.Sprintf("FAIL: probe error: %s", probeErr)
						}
					}
				}
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'freshservice-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'freshservice-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'freshservice-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'freshservice-pp-cli sync' to refresh."
	}
	return report
}
