...
Resource scoping:
  ...
  tenderned-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  tenderned-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  tenderned-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  tenderned-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  tenderned-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("tenderned-pp-cli")
			}
