...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "e-boekhouden-pp-cli", "http")
	...
	c.session = newSessionManager(httpClient, c.BaseURL)
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "e-boekhouden-pp-cli/v1")
		}
		...
		// Session token rejected mid-run (revoked, or expired earlier than
		// the cached expiresIn promised) - re-exchange once and retry the
		// same request with a fresh session token. path guard avoids a
		// retry loop against /v1/session itself, which never carries our
		// session auth header in the first place.
		if resp.StatusCode == 401 && !sessionRetried && path != "/v1/session" && c.session != nil {
			sessionRetried = true
			c.session.Invalidate()
			fresh, freshErr := c.authHeader(ctx)
			if freshErr == nil && fresh != "" && fresh != authHeader {
				authHeader = fresh
				lastErr = apiErr
				continue
			}
...
// authHeader returns the short-lived e-Boekhouden SESSION token (not the
// long-lived EBOEKHOUDEN_API_TOKEN, which is only ever sent to POST
// /v1/session). c.Config.AuthHeader() resolves the raw api-token env var,
// but the wire protocol requires exchanging it first — see session.go.
func (c *Client) authHeader(ctx context.Context) (string, error) {
	if c.Config == nil {
		return "", nil
	}
	apiToken := c.Config.AuthHeader()
	if apiToken == "" {
		return "", nil
	}
	if authHeaderLooksLikePlaceholderCredential(apiToken) {
		return "", authPlaceholderCredentialError(c.Config)
	}
	if c.session == nil {
		c.session = newSessionManager(c.HTTPClient, c.BaseURL)
	}
	return c.session.EnsureToken(ctx, apiToken)
}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export EBOEKHOUDEN_API_TOKEN=<your-token> or e-boekhouden-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.EboekhoudenApiToken)
	}
