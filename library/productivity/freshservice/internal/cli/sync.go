...
Exit codes & warnings:
  ...
  freshservice-pp-cli sync

  # Sync specific resources only
  freshservice-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  freshservice-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  freshservice-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  freshservice-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  freshservice-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "assets_contracts":
		return db.UpsertAssetsContracts(data)
	case "requests":
		return db.UpsertRequests(data)
	case "assets_restore":
		return db.UpsertAssetsRestore(data)
	case "changes_notes":
		return db.UpsertChangesNotes(data)
	case "changes_tasks":
		return db.UpsertChangesTasks(data)
	case "changes_time_entries":
		return db.UpsertChangesTimeEntries(data)
	case "convert_to_agent":
		return db.UpsertConvertToAgent(data)
	case "reactivate":
		return db.UpsertReactivate(data)
	case "activities":
		return db.UpsertActivities(data)
	case "conversations":
	...
	case "tickets_restore":
		return db.UpsertTicketsRestore(data)
	case "tickets_tasks":
		return db.UpsertTicketsTasks(data)
	case "tickets_time_entries":
		return db.UpsertTicketsTimeEntries(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"agent-fields":       "/agent_fields",
		"agents":             "/agents",
		"assets":             "/assets",
		"canned-responses":   "/canned_responses",
		"change-form-fields": "/change_form_fields",
		"changes":            "/changes",
		"contracts":          "/contracts",
		...
		"locations":          "/locations",
		"products":           "/products",
		"requester-fields":   "/requester_fields",
		"requesters":         "/requesters",
		"service-catalog":    "/service_catalog/items",
		"solutions":          "/solutions/categories",
		"ticket-form-fields": "/ticket_form_fields",
		"tickets":            "/tickets",
		"vendors":            "/vendors",
